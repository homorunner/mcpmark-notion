...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    for block in all_blocks:
        if block.get("type") == "heading_1":
            ...
            if "Created 2025-01-19" in para_text:
                ...
                verification_results.append("✅ Responsible department updated correctly")
            ...
            try:
                ...
                child_page_title = ""
                if "properties" in child_page_info and "title" in child_page_info["properties"]:
            ...
            for child_block in child_blocks:
                if child_block.get("type") == "callout":
                    ...
                    if ("DevOps Engineering Team Wiki" in callout_text and 
                        "deployment schedules" in callout_text and
                        "deployment activities" in callout_text):
                        header_callout_found = True
                        verification_results.append(f"✅ Header People team page callout updated correctly")
            ...
            if "Purpose" in heading_text:
                ...
                for j in range(i + 1, min(i + 5, len(all_blocks))):
                    ...
                    if next_block.get("type") == "paragraph":
                        ...
                        if expected_purpose in para_text and "engineering teams" in para_text:
                            purpose_found = True
                            verification_results.append("✅ Purpose section content updated correctly")
                        break
                break
    ...
    expected_child_callouts = [
        ("Change Management Policy (SOP-001)", "Defines approval workflows and change review processes for all production modifications", "Contacting IT"),
        ("Incident Response Procedures (SOP-003)", "Emergency procedures for handling deployment failures and system outages", "Team lunches"),
        ("Security Compliance Guidelines (SOP-007)", "Security requirements and validation steps for production deployments", "Sending swag")
    ]
            ...
            if "Context" in heading_text:
                ...
                for j in range(i + 1, min(i + 10, len(all_blocks))):
                    ...
                    if next_block.get("type") == "paragraph":
                        ...
                        toggle_blocks = notion_utils.get_all_blocks_recursively(notion, next_block["id"])
                        for toggle_child in toggle_blocks:
                            if toggle_child.get("type") == "child_page":
                                ...
                                try:
                                    child_page_info = notion.pages.retrieve(page_id=toggle_child["id"])
                                    child_page_title = ""
                                    if "properties" in child_page_info and "title" in child_page_info["properties"]:
                                        title_list = child_page_info["properties"]["title"].get("title", [])
                                        if title_list:
                                ...
                                child_blocks = notion_utils.get_all_blocks_recursively(notion, toggle_child["id"])
                                for child_block in child_blocks:
                                    if child_block.get("type") == "callout":
                                        callout_text = notion_utils.get_block_plain_text(child_block)
                                        for expected_title, expected_content, expected_page_title in expected_child_callouts:
                                            if (expected_title in callout_text and 
                                                expected_content in callout_text and
                                                expected_page_title in child_page_title):
                                                child_pages_updated += 1
                                                verification_results.append(f"✅ Context child_page '{expected_page_title}' updated correctly")
                                                break
    ...
    if child_pages_updated == 3:
        verification_results.append("✅ All 3 Context child_page callouts updated correctly")
    else:
        verification_results.append(f"❌ Only {child_pages_updated}/3 Context child_page callouts updated correctly (Contacting IT, Team lunches, Sending swag)")
        ...
        if block.get("type") == "heading_2":
            ...
            if "Terminologies" in heading_text:
                ...
                for j in range(i + 1, min(i + 10, len(all_blocks))):
                    ...
                    elif next_block.get("type") in ["heading_1", "heading_2", "heading_3"]:
                        break
                break
    
    terminology_matches = sum(1 for expected in expected_terminologies 
    ...
    if terminologies_found and len(terminology_items) == 4 and terminology_matches == 4:
        verification_results.append("✅ Terminologies section with exactly 4 correct items")
    else:
        verification_results.append(f"❌ Terminologies: expected 4 items, found {len(terminology_items)}, {terminology_matches} correct")
            ...
            if "Tools" in heading_text:
                ...
                for j in range(i + 1, min(i + 10, len(all_blocks))):
                    ...
                    if next_block.get("type") == "child_page":
                        child_blocks = notion_utils.get_all_blocks_recursively(notion, next_block["id"])
                        for child_block in child_blocks:
                            if child_block.get("type") == "callout":
                                callout_text = notion_utils.get_block_plain_text(child_block)
                                for expected_title, expected_content in expected_tools:
                                    if expected_title in callout_text and expected_content in callout_text:
                                        tools_child_pages += 1
                                        break
                    elif next_block.get("type") in ["heading_1", "heading_2", "heading_3"]:
                        break
    ...
    if tools_found and tools_child_pages == 2:
        verification_results.append("✅ Tools section with 2 correctly updated child_page callouts")
    else:
        verification_results.append(f"❌ Tools section: expected 2 child_pages updated, found {tools_child_pages}")
    ...
    role_matches = sum(1 for expected in expected_roles 
                      if any(expected in item for item in role_items))
    
    if roles_found and len(role_items) == 4 and role_matches == 4:
        verification_results.append("✅ Roles & responsibilities section with exactly 4 correct items")
    else:
        verification_results.append(f"❌ Roles section: expected 4 items, found {len(role_items)}, {role_matches} correct")
    ...
    expected_procedures = [
        ...
        ("Post-deployment", "minimum 30 minutes", "stakeholders via deployment notification")
    ]
    ...
    for item_text in procedure_items:
        for expected_title, expected_content1, expected_content2 in expected_procedures:
            if (expected_title in item_text and 
                expected_content1 in item_text and 
                expected_content2 in item_text):
                procedure_matches += 1
                break
    ...
    if (procedure_found and len(procedure_items) == 3 and procedure_matches == 3):
        ...
        verification_results.append(f"❌ Procedure: expected 3 items, found {len(procedure_items)}, {procedure_matches} correct")
    ...
    success = (
        sop_title_found and created_date_found and responsible_dept_found and header_callout_found and
        purpose_found and context_found and child_pages_updated == 3 and
        terminologies_found and len(terminology_items) == 4 and terminology_matches == 4 and
        tools_found and tools_child_pages == 2 and
        roles_found and len(role_items) == 4 and role_matches == 4 and
    ...
    if success:
        ...
        print(f"\n❌ FAILURE: SOP template verification failed. {successful_checks}/{total_checks} requirements met.", file=sys.stderr)
        return False
