...
def verify_todo_database_correspondence(all_blocks, activities_by_day, _):
    ...
    for day in ["Day 1", "Day 2", "Day 3"]:
        ...
        if len(db_activities) != len(page_todos):
            print(f"Error: {day} activity count mismatch. Database has {len(db_activities)} activities, page has {len(page_todos)} to-dos.", file=sys.stderr)
            return False
        ...
        for db_activity in db_activities:
            ...
            for todo in page_todos:
                if expected_format in todo['text'] or db_activity['name'] in todo['text']:
                    matching_todo = todo
                    break
            
            if not matching_todo:
                print(f"Error: {day} - Database activity '{expected_format}' not found in to-do list.", file=sys.stderr)
                return False
            ...
            if db_activity['visited'] != matching_todo['checked']:
                ...
                print(f"Error: {day} - Activity '{db_activity['name']}' should be {status_desc} but is {actual_desc}.", file=sys.stderr)
                return False
    ...
    for block in all_blocks:
        if block.get("type") == "paragraph":
            block_text = notion_utils.get_block_plain_text(block)
            if f"Total activities visited (from Day 1 to Day 3): 8" in block_text:
                print(f"Success: Daily Itinerary Overview page created with correct structure. All {checked_todos_count} visited activities match database.")
                return True
    
    print(f"Error: Summary shows incorrect visited activity count. Expected: {checked_todos_count} (based on checked to-do items)", file=sys.stderr)
    return False
...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    try:
        ...
        print(f"Error searching for Daily Itinerary Overview page: {e}", file=sys.stderr)
        return False

    if not overview_page_id:
        print("Error: 'Daily Itinerary Overview' page not found as child of main page.", file=sys.stderr)
        return False
    ...
    for i, (found_heading, found_type) in enumerate(found_headings_in_order):
        ...
        if found_heading != expected_heading or found_type != expected_type:
            print(f"Error: Headings not in correct order. Expected '{expected_heading}' ({expected_type}) at position {i+1}, but found '{found_heading}' ({found_type})", file=sys.stderr)
            return False
    ...
    if not summary_has_correct_format:
        print("Error: Trip summary does not have correct format 'Total activities visited: [NUMBER]'.", file=sys.stderr)
        return False

    # Verify to-do list items exist (activities should be in to-do format)
    if not found_todo_items:
        print("Error: No to-do list items found. Activities should be listed as to-do items under day headings.", file=sys.stderr)
        return False

    # Additional verification: Check if Travel Itinerary database exists and has data
    try:
        itinerary_db_id = notion_utils.find_database_in_block(notion, page_id, "Travel Itinerary")
        ...
        if itinerary_db_id:
            ...
            for result in db_activities:
                ...
                for prop_name, prop_value in properties.items():
                    ...
                    # Get city info
                    elif "city" in prop_name.lower() and prop_type in ["rich_text", "select"]:
                        if prop_type == "rich_text" and prop_value.get("rich_text"):
                            activity_info["city"] = prop_value["rich_text"][0]["plain_text"]
                    ...
                    # Get day info
                    elif "day" in prop_name.lower() and prop_type in ["select", "rich_text"]:
            ...
            return verify_todo_database_correspondence(all_blocks, activities_by_day, visited_count)
        else:
            print("Warning: Travel Itinerary database not found, using to-do items for count verification.")
            ...
            for block in all_blocks:
                if block.get("type") == "paragraph":
                    ...
                    if f"Total activities visited: {checked_todos_count}" in block_text:
                        print(f"Success: Daily Itinerary Overview page created with correct structure and {checked_todos_count} visited activities.")
                        return True
