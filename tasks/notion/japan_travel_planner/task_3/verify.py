...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    places_db_id = notion_utils.find_database_in_block(notion, page_id, "Travel Itinerary")
    if not places_db_id:
    ...
    if not itinerary_results:
        print("Error: No restaurants found for Day 1 in Travel Itinerary.", file=sys.stderr)
        return False
    ...
    try:
        places_results = notion.databases.query(database_id=places_db_id).get("results", [])
    except Exception as e:
    ...
    for place in places_results:
        ...
        desc_text = "".join(t.get("plain_text", "") for t in desc_prop.get("rich_text", []))
    ...
    try:
        expenses_results = notion.databases.query(database_id=expenses_db_id).get("results", [])
    except Exception as e:
    ...
    for restaurant_name in restaurant_names:
        ...
        for expense in expenses_results:
            ...
            expense_text = "".join(t.get("plain_text", "") for t in expense_prop.get("title", []))
            ...
            if expected_description:
                ...
                comment_text = "".join(t.get("plain_text", "") for t in comment_prop.get("rich_text", []))
                if comment_text.strip().replace("\u202f", " ") != expected_description.replace("\u202f", " "):
                    continue
        ...
        if not found_matching_expense:
            print(f"Error: No matching expense entry found for restaurant '{restaurant_name}'.", file=sys.stderr)
            return False

    if len(verified_restaurants) == len(restaurant_names):
        print(f"Success: Found matching expense entries for all {len(restaurant_names)} Day 1 restaurants.")
        return True
    else:
        print(f"Error: Only {len(verified_restaurants)} out of {len(restaurant_names)} restaurants have matching expense entries.", file=sys.stderr)
        return False
