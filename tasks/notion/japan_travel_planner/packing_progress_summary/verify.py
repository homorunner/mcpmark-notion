...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    try:
        ...
        for page in db_items.get("results", []):
            ...
            if type_prop.get("type") == "multi_select":
                types = [opt.get("name", "") for opt in type_prop.get("multi_select", [])]
        ...
        for item in clothes_items:
            if item["is_hat"]:
                if item["packed"]:
                    print(f"Error: Hat should not be packed but is marked as packed.", file=sys.stderr)
                    return False
            else:
                if not item["packed"]:
                    print(f"Error: Clothes item '{item['name']}' should be packed but is not.", file=sys.stderr)
                    return False
    ...
    if packing_heading_index is not None:
        ...
        for i in range(packing_heading_index + 1, min(packing_heading_index + 15, len(all_blocks))):
            block = all_blocks[i]
            ...
            if "Packing Progress Summary" in block_text:
                ...
                if block.get("type") == "paragraph":
                    ...
                    for text_obj in rich_text_list:
                        if "Packing Progress Summary" in text_obj.get("text", {}).get("content", ""):
                            if not text_obj.get("annotations", {}).get("bold", False):
                                print("Error: 'Packing Progress Summary' text is not bold.", file=sys.stderr)
                                return False
            
            # Check for statistics bullet points in format "Category: X/Y packed"
            if block.get("type") == "bulleted_list_item" and ":" in block_text and "/" in block_text and "packed" in block_text:
    ...
    if not summary_found:
        print("Error: 'Packing Progress Summary' section not found after Packing List heading.", file=sys.stderr)
        return False
    
    if not found_statistics:
        print("Error: No valid packing statistics bullet points found in format 'Category: X/Y packed'.", file=sys.stderr)
        return False
    ...
    for category, stats in expected_stats.items():
        if category not in found_statistics:
            print(f"Error: Category '{category}' missing from Packing Progress Summary.", file=sys.stderr)
            statistics_verified = False
        else:
            found = found_statistics[category]
            if found["packed"] != stats["packed"] or found["total"] != stats["total"]:
                print(f"Error: Statistics mismatch for '{category}': expected {stats['packed']}/{stats['total']} packed, found {found['packed']}/{found['total']} packed.", file=sys.stderr)
                statistics_verified = False
