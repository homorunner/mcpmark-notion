...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    for block in all_blocks:
        if (block.get("type") == "column_list" and 
            block.get("parent", {}).get("block_id") == faq_toggle_id):
    ...
    direct_faq_children = []
    for block in all_blocks:
        if (block.get("parent", {}).get("block_id") == faq_toggle_id and 
            block.get("id") != column_list_block.get("id")):
    ...
    for block in direct_faq_children:
        if block.get("type") in ["heading_3", "paragraph"]:
            print(f"Error: Found Q&A content outside column_list: {notion_utils.get_block_plain_text(block)[:50]}...", file=sys.stderr)
            return False
    ...
    for block in all_blocks:
        if (block.get("type") == "column" and 
            block.get("parent", {}).get("block_id") == column_list_id):
    ...
    for i, column in enumerate(columns):
        ...
        while j < len(column_blocks):
            if (column_blocks[j].get("type") == "heading_3" and 
                j + 1 < len(column_blocks) and 
                column_blocks[j + 1].get("type") == "paragraph"):
                qa_pairs += 1
        ...
        if qa_pairs != 2:
            print(f"Error: Column {i+1} has {qa_pairs} Q&A pairs, expected 2.", file=sys.stderr)
            return False
        
        print(f"Column {i+1}: Found {qa_pairs} Q&A pairs ✓")
    
    print("Success: FAQ toggle properly organized with 2 columns, each containing 2 Q&A pairs.")
    return True
