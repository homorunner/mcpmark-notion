...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    all_blocks = notion_utils.get_all_blocks_recursively(notion, self_assessment_page_id)
    ...
    if len(numbered_list_items) > 0:
        print(f"Error: found {len(numbered_list_items)} numbered list items that should be converted to emoji numbers", file=sys.stderr)
        # return False
