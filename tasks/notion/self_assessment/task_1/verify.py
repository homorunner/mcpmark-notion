...
def verify(notion: Client, main_id: str = None) -> bool:
    ...
    callout_position = -1
    divider_position = -1
    children = notion.blocks.children.list(block_id=self_assessment_page_id).get("results", [])
    for i, child in enumerate(children):
        ...
        # Find the report page
            ...
            title_prop = page_data.get("properties", {}).get("title", {}).get("title", [])
            if title_prop and title_prop[0].get("plain_text") == "Hyperfocus Analysis Report":
    ...
    if callout_position == -1:
        print("Error: Could not find 'Why Use the Term \"Hyperfocus\"?' callout.", file=sys.stderr)
        return False
    ...
    if not (callout_position < report_position < divider_position):
        print(f"Error: Report page is not positioned between callout and divider. Positions: callout={callout_position}, report={report_position}, divider={divider_position}", file=sys.stderr)
        return False
    ...
    database_id = None
    for block in notion_utils.get_all_blocks_recursively(notion, self_assessment_page_id):
        if block.get("type") == "child_database":
            db_data = notion.databases.retrieve(database_id=block["id"])
            db_title = "".join([t.get("plain_text", "") for t in db_data.get("title", [])])
            if "Hyperfocus Self-Assessment Worksheet" in db_title:
                database_id = block["id"]
                break
    
    if not database_id:
        print("Error: Database 'Hyperfocus Self-Assessment Worksheet' not found.", file=sys.stderr)
        return False
    ...
    query_results = notion.databases.query(
        ...
        filter={
            "and": [
                {
                    "property": "Work Completion Rate",
                    "number": {"greater_than": 0.8}
                },
                {
                    "property": "Challenges",
    ...
    if not query_results:
        print("Warning: No sessions found with >80% completion rate and challenges.", file=sys.stderr)
    ...
    for session in all_sessions:
        strategies = session.get("properties", {}).get("Key Strategies Used", {}).get("multi_select", [])
    ...
    for result in query_results:
        ...
        activity_prop = result.get("properties", {}).get("Activity", {}).get("select", {})
        if date_prop and date_prop.get("start") and activity_prop:
            ...
            focus_factors = [f.get("name", "") for f in result.get("properties", {}).get("Focus Factors", {}).get("multi_select", [])]
            challenges = [c.get("name", "") for c in result.get("properties", {}).get("Challenges", {}).get("multi_select", [])]
            strategies = [s.get("name", "") for s in result.get("properties", {}).get("Key Strategies Used", {}).get("multi_select", [])]
            energy = result.get("properties", {}).get("Energy Level", {}).get("number")
            mood = result.get("properties", {}).get("Mood", {}).get("number")
            completion = result.get("properties", {}).get("Work Completion Rate", {}).get("number")
        ...
        if block_type == "bulleted_list_item" and current_session_data:
            ...
            if bullet_text.startswith("Focus factors"):
                ...
                if not validate_comma_separated(content, expected_factors):
                    print(f"Error: Focus factors mismatch for {current_session_date}. Expected: {expected_factors}, Found: {content}", file=sys.stderr)
                    return False
                ...
                if energy_match and mood_match:
                    ...
                    if found_energy != expected_energy or found_mood != expected_mood:
                        print(f"Error: Energy/Mood mismatch for {current_session_date}. Expected: Energy: {expected_energy}/10, Mood: {expected_mood}/10", file=sys.stderr)
                        return False
                else:
                    print(f"Error: Invalid Energy/Mood format for {current_session_date}", file=sys.stderr)
                    return False
                ...
                if not validate_comma_separated(content, expected_challenges):
                    print(f"Error: Challenges mismatch for {current_session_date}. Expected: {expected_challenges}, Found: {content}", file=sys.stderr)
                    return False
            
            elif bullet_text.startswith("Strategies"):
                content = bullet_text.split(":", 1)[1].strip()
                expected_strategies = current_session_data.get("strategies", [])
                if len(expected_strategies) > 0 and not validate_comma_separated(content, expected_strategies):
                    print(f"Error: Strategies mismatch for {current_session_date}. Expected: {expected_strategies}, Found: {content}", file=sys.stderr)
                    return False
                ...
                if completion_match:
                    ...
                    expected_completion = int(current_session_data.get("completion", 0) * 100)
                    
                    if found_completion != expected_completion:
                        print(f"Error: Completion rate mismatch for {current_session_date}. Expected: {expected_completion}%, Found: {found_completion}%", file=sys.stderr)
                        return False
                else:
                    print(f"Error: Invalid completion format for {current_session_date}", file=sys.stderr)
                    return False
    ...
    for date_str, bullets in session_bullet_points.items():
        ...
        required_items = ["Focus factors", "Energy:", "Mood:", "Challenges", "Strategies", "Completion"]
        missing_items = []
        ...
        if missing_items:
            print(f"Error: Missing bullet points for session {date_str}: {', '.join(missing_items)}", file=sys.stderr)
            return False
    
    # Verify all requirements
    if not has_callout:
        print("Error: Missing callout block with 'Top 2 Most Effective Strategies'.", file=sys.stderr)
        return False
    ...
    if missing_sessions:
        print(f"Error: Missing session sections for dates: {', '.join(missing_sessions)}", file=sys.stderr)
        return False
    
    if query_results and session_count < len(query_results):
        print(f"Warning: Found {session_count} session sections but expected {len(query_results)}.", file=sys.stderr)
    
    print("Success: Hyperfocus Analysis Report created with proper structure and content.")
    return True
