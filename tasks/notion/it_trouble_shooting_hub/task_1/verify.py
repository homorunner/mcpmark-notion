...
def verify(notion: Client, main_id: str | None = None) -> bool:
    ...
    # ----------------------------------------------------------------------------------
    requests_db_id = notion_utils.find_database_in_block(notion, root_page_id, "IT Requests")
    if not requests_db_id:
        print("Error: 'IT Requests' database not found in the workspace.", file=sys.stderr)
        return False
    ...
    if not target_page:
        print(f"Failure: Ticket with title '{expected_title}' was not found in 'IT Requests' database.", file=sys.stderr)
        return False
    ...
    if priority_value != "High":
        print(f"Failure: Expected Priority 'High', found '{priority_value}'.", file=sys.stderr)
        return False
    ...
    if not due_date_start or not due_date_start.startswith(expected_due_iso):
        print(f"Failure: Expected Due date '{expected_due_iso}', found '{due_date_start}'.", file=sys.stderr)
        return False
    ...
    bullet_texts = [notion_utils.get_block_plain_text(b) for b in blocks if b.get("type") == "bulleted_list_item"]
    ...
    if len(bullet_texts) != len(expected_items):
        print(f"Failure: Expected {len(expected_items)} bullet items, found {len(bullet_texts)}.", file=sys.stderr)
        return False
    ...
    for text in bullet_texts:
        ...
        if not m:
            print(f"Failure: Bullet item '{text}' does not follow '<Serial> - <Tag> - <Recommendation>' format.", file=sys.stderr)
            return False
        serial, tag, advice = m.group(1).strip(), m.group(2).strip(), m.group(3).strip()
        if serial not in expected_items:
            print(f"Failure: Unexpected Serial '{serial}' found in bullet list.", file=sys.stderr)
            return False
        if expected_items[serial] != tag:
            print(f"Failure: Serial '{serial}' expected tag '{expected_items[serial]}', found '{tag}'.", file=sys.stderr)
            return False
        if not advice:
            print(f"Failure: Bullet item for Serial '{serial}' is missing a recommendation/advice.", file=sys.stderr)
            return False
        matched.add(serial)

    if len(matched) != len(expected_items):
        missing = set(expected_items.keys()) - matched
        print(f"Failure: Missing bullet items for serials: {', '.join(missing)}.", file=sys.stderr)
        return False
