...
def verify(notion: Client) -> bool:
    ...
    def find_image_url_under_heading(blocks, heading_text, notion_client):
        ...
        for i in range(heading_index + 1, len(blocks)):
            ...
            if block.get("type") == "column_list":
                ...
                columns = notion_utils.get_all_blocks_recursively(notion_client, column_list_id)
                for column in columns:
                    if column.get("type") == "column":
                        column_id = column["id"]
                        column_blocks = notion_utils.get_all_blocks_recursively(notion_client, column_id)
                        for inner_block in column_blocks:
                            if inner_block.get("type") == "image" and inner_block.get("image", {}).get("file"):
                                return inner_block.get("image", {}).get("file", {}).get("url")
        return None
    ...
    if not education_image_url:
        print("Error: Could not find the image in the 'Education' section.", file=sys.stderr)
        return False

    heading_text = "Work History"
    heading_index = -1
    for i, block in enumerate(all_blocks):
        if block.get("type") == "heading_1" and heading_text in notion_utils.get_block_plain_text(block):
            heading_index = i
            break
    ...
    for i in range(heading_index + 1, len(all_blocks)):
        ...
        if block.get("type") == "column_list":
            ...
            image_column_blocks = notion_utils.get_all_blocks_recursively(notion, image_column["id"])
            text_column_blocks = notion_utils.get_all_blocks_recursively(notion, text_column["id"])
            ...
            for j, inner_block in enumerate(text_column_blocks):
                ...
                if "Research Assistant" in notion_utils.get_block_plain_text(inner_block):
                    ...
                    if j + 2 < len(text_column_blocks):
                        ...
                        description_annotations = get_block_annotations(description_block)
