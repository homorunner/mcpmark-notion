    ...
    for i in range(1, 21):
        ...
        for start_pos in range(len(file_content)):
            for end_pos in range(
                start_pos + 30, len(file_content) + 1
    ...
    try:
        ...
        for filename, start_pos in answer_matches.items():
            if filename not in expected_matches:
                print(
                    f"❌ File {filename} listed in answer but has no valid 30+ character match"
                )
                return False
...
def verify_match_length_is_30_plus(test_dir: Path) -> bool:
    ...
    try:
        ...
        for line in lines:
            ...
            longest_match = ""
            for end_pos in range(
                start_int + 30 - 1, len(large_content) + 1
            ):  # At least 30 characters
                substring = large_content[
                    start_int - 1 : end_pos
            ...
            if len(longest_match) < 30:
                print(
                    f"❌ Match in {filename} is {len(longest_match)} characters, less than 30"
                )
...
def main():
    ...
    # test_dir = Path("/Users/chenlingjun/Desktop/MCP/MCPMark/.mcpmark_backups/backup_filesystem_file_context_file_filtering_18503")
