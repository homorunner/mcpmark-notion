...
def verify_no_duplicate_files_in_original(test_dir: Path) -> bool:
    ...
    if remaining_duplicates:
        print(
            f"❌ Duplicate files still exist in original location: {remaining_duplicates}"
        )
        return False
    ...
    for group_name, files in expected_duplicate_groups.items():
        ...
        for filename in files[1:]:
            ...
            if file_hash != first_hash:
                print(
                    f"❌ Files in group {group_name} have different content: {files[0]} vs {filename}"
                )
                return False
...
def verify_total_file_count(test_dir: Path) -> bool:
    ...
    if actual_duplicates != expected_duplicates:
        print(
            f"❌ Wrong number of files in duplicates directory. Expected: {expected_duplicates}, Actual: {actual_duplicates}"
        )
        return False
...
def main():
    ...
    # test_dir = Path("/Users/chenlingjun/Desktop/MCP/MCPMark/.mcpmark_backups/backup_filesystem_file_context_duplicates_checking_21630")
