...
def verify_file_classification(test_dir: Path) -> bool:
    ...
    for dir_name, expected_files in expected_classification.items():
        ...
        unexpected_files = [
            f for f in actual_files if f not in expected_files and f not in system_files
        ]
        ...
        for file_path in dir_path.iterdir():
            if file_path.is_file():
                ...
                if dir_name == "small_files" and file_size >= 300:
                    print(
                        f"❌ File {file_path.name} in small_files but size is {file_size} bytes"
                    )
                    return False
                elif dir_name == "medium_files" and (
                    file_size < 300 or file_size > 700
                ):
                    print(
                        f"❌ File {file_path.name} in medium_files but size is {file_size} bytes"
                    )
                    return False
                elif dir_name == "large_files" and file_size <= 700:
                    print(
                        f"❌ File {file_path.name} in large_files but size is {file_size} bytes"
                    )
                    return False
...
def main():
    ...
    try:
        ...
        # test_dir = Path("/Users/chenlingjun/Desktop/MCP/MCPMark/.mcpmark_backups/backup_filesystem_file_property_file_classification_74255")
