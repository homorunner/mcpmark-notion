...
def verify_zero123_guidance_content(test_dir: Path) -> bool:
    ...
    try:
        ...
        if not is_main_implementation:
            print("❌ File is not the main Zero123 guidance implementation")
            print(
                "   Expected: class Zero123Guidance and @threestudio.register('zero123-guidance')"
            )
            return False

        print(
            f"✅ File contains main Zero123 guidance implementation indicators: {found_indicators}"
        )
        return True
