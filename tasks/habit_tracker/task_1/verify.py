...
def verify(notion: Client) -> bool:
    ...
    if not db_id:
        print("Error: Habit Tracker database not found within the page.", file=sys.stderr)
        return False
    ...
    for entry in results:
        ...
        for day in EXPECTED_CHECKBOX_DAYS:
            ...
            if not day_prop or day_prop.get("type") != "checkbox":
                print(f"Error: Property '{day}' missing or not a checkbox for habit entry.", file=sys.stderr)
                return False
            if not day_prop.get("checkbox", False):
                print(f"Error: Checkbox for '{day}' not marked true for habit entry.", file=sys.stderr)
                return False
