...
def verify_task() -> bool:
    ...
    EXPECTED_TOP_COLLABORATORS = [
        {"username": "bcherny", "min_collaborations": 14},    # Boris Cherny has many Claude collaborations
    ...
    if stats.get('total_analyzed') != EXPECTED_STATS['total_analyzed']:
        print(f"Error: Total analyzed should be {EXPECTED_STATS['total_analyzed']}, found {stats.get('total_analyzed')}", file=sys.stderr)
        return False
    
    if stats.get('claude_commits') != EXPECTED_STATS['claude_commits']:
        print(f"Error: Claude commits should be {EXPECTED_STATS['claude_commits']}, found {stats.get('claude_commits')}", file=sys.stderr)
        return False
    
    if stats.get('percentage') != EXPECTED_STATS['percentage']:
        print(f"Error: Percentage should be {EXPECTED_STATS['percentage']}%, found {stats.get('percentage')}%", file=sys.stderr)
        return False
        
    if stats.get('unique_collaborators') != EXPECTED_STATS['unique_collaborators']:
        print(f"Error: Unique collaborators should be {EXPECTED_STATS['unique_collaborators']}, found {stats.get('unique_collaborators')}", file=sys.stderr)
        return False
    ...
    if len(collaborators) < 3:
        print(f"Error: Expected 3 top collaborators, found {len(collaborators)}", file=sys.stderr)
        return False
            ...
            for collab in collaborators:
                if collab['username'] == expected['username']:
                    if collab['collaborations'] < expected['min_collaborations']:
                        print(f"Error: {expected['username']} should have at least {expected['min_collaborations']} collaborations, found {collab['collaborations']}", file=sys.stderr)
                        return False
    
    if expected_found < 2:
        print(f"Error: Expected to find at least 2 of the known top collaborators in top 3, found {expected_found}", file=sys.stderr)
        print(f"Expected to see at least 2 of: {[e['username'] for e in EXPECTED_TOP_COLLABORATORS]}", file=sys.stderr)
    ...
    if not commit_found:
        print(f"Error: Expected commit message '{expected_commit_message}' not found in recent commits", file=sys.stderr)
        return False
    ...
    print(f"Claude collaboration analysis completed successfully:")
    print(f"  - File: CLAUDE_COLLABORATION_ANALYSIS.md created in main branch")
    ...
    print(f"  - Commit message verified")
