...
def verify() -> bool:
    ...
    if not _check_file_sections(changelog_content, CHANGELOG_SECTIONS):
        print("Error: CHANGELOG-GENERATED.md missing required sections", file=sys.stderr)
        return False
    ...
    if bug_refs < 8:  # At least 8 of the bug issues
        print(f"Error: CHANGELOG-GENERATED.md only references {bug_refs} bug issues, expected at least 8", file=sys.stderr)
        return False
    
    # Check for platform and area statistics
    if "platform:" not in changelog_content.lower() or "area:" not in changelog_content.lower():
        print("Error: CHANGELOG-GENERATED.md missing platform or area distribution", file=sys.stderr)
        return False
    ...
    if pr_refs < 3:
        print(f"Error: MIGRATION_GUIDE.md only references {pr_refs}/3 open PRs", file=sys.stderr)
        return False
    ...
    if "#50" not in issue_analysis_content and "#48" not in issue_analysis_content:
        print("Warning: ISSUE_ANALYSIS.md may be missing cross-project issue references", file=sys.stderr)
    ...
    if not _check_file_sections(pr_plan_content, PR_INTEGRATION_SECTIONS):
        print("Error: PR_INTEGRATION_PLAN.md missing required sections", file=sys.stderr)
        return False
    ...
    if pr_refs_in_plan < 3:
        print(f"Error: PR_INTEGRATION_PLAN.md only references {pr_refs_in_plan}/3 open PRs", file=sys.stderr)
        return False
    ...
    required_sections = ["## Summary", "## Files Created", "## Issues Processed", "## PRs Analyzed"]
    missing_sections = []
    ...
    if len(missing_sections) > 1:  # Allow 1 missing section for flexibility
        print(f"Error: Documentation PR missing sections: {missing_sections}", file=sys.stderr)
        return False
    ...
    if merge_commit:
        ...
        if len(parents) != 1:
            print(f"Warning: Merge commit has {len(parents)} parents, may not be squash merge", file=sys.stderr)
        ...
        if f"#{pr_number}" not in commit_message:
            print("Warning: Merge commit message may not follow squash merge pattern", file=sys.stderr)
    else:
    ...
    print(f"Changelog and migration documentation completed successfully:")
    print(f"  - Documentation PR #{pr_number} (merged)")
    print(f"  - Branch: {DOCS_BRANCH_NAME}")
    print(f"  - Files created: 4 documentation files")
