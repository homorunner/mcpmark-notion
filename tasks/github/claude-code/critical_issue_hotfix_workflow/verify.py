...
def _check_headings_and_keywords(body: str, headings: List[str], keywords: List[str]) -> bool:
...
def verify() -> bool:
    ...
    TRACKING_ISSUE_TITLE_KEYWORDS = ["CRITICAL", "Memory", "Context Management", "Hotfix Tracking"]
    TRACKING_ISSUE_REFERENCE_NUMBERS = ["49", "46", "47"]
    TRACKING_ISSUE_HEADINGS = ["## Critical Issues", "## Impact Assessment", "## Resolution Strategy"]
    TRACKING_ISSUE_KEYWORDS = ["memory exhaustion", "context auto-compact", "JavaScript heap", "hotfix priority"]
    
    # PR content requirements  
    HOTFIX_PR_TITLE_KEYWORDS = ["HOTFIX", "Critical memory optimization", "issues #49", "#46"]
    ...
    if not _check_exact_file_content(memory_doc_content, MEMORY_DOC_SECTIONS):
        print("Error: MEMORY_OPTIMIZATION.md missing required sections or content", file=sys.stderr)
        return False
    ...
    if not _check_headings_and_keywords(tracking_issue_body, TRACKING_ISSUE_HEADINGS, TRACKING_ISSUE_KEYWORDS):
        print("Error: Tracking issue missing required headings or keywords", file=sys.stderr)
        return False
    
    if not _check_issue_references(tracking_issue_body, TRACKING_ISSUE_REFERENCE_NUMBERS):
        print("Error: Tracking issue does not reference required issues #49, #46, #47", file=sys.stderr)
        return False
    ...
    if not _check_headings_and_keywords(hotfix_pr_body, HOTFIX_PR_HEADINGS, HOTFIX_PR_KEYWORDS):
    ...
    if not _check_addresses_pattern(hotfix_pr_body, HOTFIX_PR_ADDRESSES_NUMBERS):
        print("Error: Hotfix PR does not properly address issues #49 and #46", file=sys.stderr)
        return False
    
    # Check reference to tracking issue
    if f"#{tracking_issue_number}" not in hotfix_pr_body:
        print(f"Error: Hotfix PR does not reference tracking issue #{tracking_issue_number}", file=sys.stderr)
        return False
    ...
    if not _check_headings_and_keywords(pr51_body, ["## Technical Implementation"], PR51_UPDATE_KEYWORDS):
        print("Error: PR #51 missing updated technical implementation section", file=sys.stderr)
        return False
    ...
    if not has_implementation_comment:
        print(f"Error: Tracking issue #{tracking_issue_number} missing implementation comment with required references and keywords", file=sys.stderr)
        return False
    ...
    print(f"Critical issue hotfix workflow completed successfully:")
