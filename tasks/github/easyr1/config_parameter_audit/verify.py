...
def _verify_commit_data(results: Dict, headers: Dict[str, str]) -> bool:
    ...
    if expected_author != actual_author:
        print(f"Error: Commit author mismatch. Expected: {expected_author}, Actual: {actual_author}", file=sys.stderr)
        return False
    
    # Verify date format
    commit_date = results.get("commit_date")
    if not re.match(r'^\d{4}-\d{2}-\d{2}$', commit_date):
        print(f"Error: Invalid date format: {commit_date}. Expected YYYY-MM-DD", file=sys.stderr)
        return False
...
def _verify_parameter_changes(results: Dict, headers: Dict[str, str]) -> bool:
    ...
    required_params = ["micro_batch_size_per_device_for_update", "micro_batch_size_per_device_for_experience"]
    for param in required_params:
        ...
        if not all(key in change_data for key in ["before", "after", "line_number"]):
            print(f"Error: Incomplete change data for parameter: {param}", file=sys.stderr)
            return False
    ...
    if update_param.get("before") != 4 or update_param.get("after") != 1:
        print(f"Error: Incorrect values for micro_batch_size_per_device_for_update", file=sys.stderr)
        return False
    
    experience_param = param_changes.get("micro_batch_size_per_device_for_experience", {})  
    if experience_param.get("before") != 16 or experience_param.get("after") != 2:
        print(f"Error: Incorrect values for micro_batch_size_per_device_for_experience", file=sys.stderr)
        return False
...
def _get_all_issues_with_keywords(headers: Dict[str, str]) -> set:
    ...
    page = 1
    while True:
        success, issues = _get_github_api(f"issues?state=all&per_page=100&page={page}", headers)
...
def _verify_issue_references(results: Dict, headers: Dict[str, str]) -> bool:
    ...
    if not isinstance(issue_number_list, list) or len(issue_number_list) == 0:
        print(f"Error: related_issue_number_list must be a non-empty list", file=sys.stderr)
        return False
    ...
    for issue_number in issue_number_list:
        if not isinstance(issue_number, int) or issue_number <= 0:
            print(f"Error: Invalid issue number format: {issue_number}", file=sys.stderr)
            return False
        
        # Get issue details
        success, issue_data = _get_github_api(f"issues/{issue_number}", headers)
        if not success:
            print(f"Error: Issue #{issue_number} not found in repository", file=sys.stderr)
            return False
        ...
        if not issue_has_keyword:
            print(f"Error: Issue #{issue_number} does not contain any required keywords: {required_keywords}", file=sys.stderr)
            return False
    ...
    if provided_issues != expected_issues:
        ...
        if missing:
            print(f"Error: Missing issues that contain required keywords: {missing}", file=sys.stderr)
        if extra:
            print(f"Error: Extra issues that don't contain required keywords: {extra}", file=sys.stderr)
        return False
    
    print(f"✓ Found all {len(issue_number_list)} issues containing required keywords: {issue_number_list}")
    return True
...
def verify() -> bool:
    ...
    print(f"- Verified parameter changes: micro_batch_size_per_device_for_update (4→1), micro_batch_size_per_device_for_experience (16→2)")
    print(f"- Verified memory/performance issue correlations: {results.get('related_issue_number_list')}")
