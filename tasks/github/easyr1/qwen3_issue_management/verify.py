...
def _search_github_issues(query: str, headers: Dict[str, str]) -> Tuple[bool, Optional[List]]:
...
def _check_qwen3_issues_reopened(headers: Dict[str, str]) -> Tuple[bool, List]:
    ...
    if issues_not_reopened:
        print(f"Error: The following Qwen3 issues are still closed:", file=sys.stderr)
        for issue in issues_not_reopened:
            print(f"  - {issue}", file=sys.stderr)
        return False, []
    
    if issues_not_tagged:
        print(f"Error: The following reopened issues are missing 'qwen3-related' label:", file=sys.stderr)
...
def _check_summary_issue(headers: Dict[str, str], reopened_issues: List) -> Optional[Dict]:
    ...
    for issue in issues:
        ...
        if title == expected_title:
            ...
            if "The following closed issues containing 'qwen3' have been reopened:" not in body:
            ...
            if "All reopened issues have been tagged with the `qwen3-related` label" not in body:
            ...
            for reopened_issue in reopened_issues:
                ...
                if f"#{issue_num}" not in body:
                    print(f"Error: Summary issue missing reference to issue #{issue_num}", file=sys.stderr)
                    return None
            
            # Check if summary issue has the label
            labels = [label.get("name") for label in issue.get("labels", [])]
            if "qwen3-related" not in labels:
                print("Error: Summary issue missing 'qwen3-related' label", file=sys.stderr)
                return None
            
            return issue
    
    print("Error: Summary issue 'Reopened Qwen3 Issues Summary' not found", file=sys.stderr)
    return None
...
def verify() -> bool:
    ...
    print(f"✓ Tagging: All reopened issues tagged with 'qwen3-related' label")
    print(f"✓ Summary: Issue #{summary_issue.get('number')} created with complete list of reopened issues")
