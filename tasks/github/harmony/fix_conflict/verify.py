...
def _find_infrastructure_pr(headers: Dict[str, str], org: str, repo: str = "harmony") -> Optional[Dict]:
    ...
    if success and prs:
        for pr in prs:
            ...
            body_ok = has_reference and has_prep_text and has_github_text and has_workflow_text
...
def verify() -> bool:
    ...
    if not infra_pr:
        ...
        print("Required body: reference with 'Fixes #' or 'Resolves #', 'prepares infrastructure', 'missing .github directory', 'workflow conflicts'", file=sys.stderr)
        return False
    ...
    if not infra_pr.get("merged_at"):
        print(f"Error: Infrastructure PR #{infra_pr.get('number')} not merged yet", file=sys.stderr)
        return False
    ...
    print(f"Infrastructure PR #{infra_pr.get('number')} extracted content from PR #24 and resolved conflicts")
    print(f"PR #24 is now merged cleanly and has a comment linking to the infrastructure PR")
    return True
