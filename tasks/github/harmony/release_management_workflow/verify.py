...
def _check_specific_file_content(branch: str, file_path: str, expected_content: str, headers: Dict[str, str], min_length: int = 100) -> bool:
...
def _check_pr_merged(title_substring: str, base_branch: str, headers: Dict[str, str]) -> Tuple[bool, Optional[int]]:
    """Check if a PR with specified title was merged into base branch and return PR number."""
    # Check closed PRs to find merged ones
    success, prs = _get_github_api("pulls?state=closed&per_page=100", headers)
...
def _check_pr_squash_merged(pr_number: int, headers: Dict[str, str]) -> bool:
...
def verify() -> bool:
    ...
    if not _check_branch_exists(RELEASE_BRANCH, headers):
    ...
    if not _check_specific_file_content("main", "src/encoding.rs", METASEP_FIX, headers, min_length=500):
    ...
    if not _check_specific_file_content("main", "src/registry.rs", REGISTRY_FIX, headers, min_length=500):
        print("Error: MetaSep registry fix not found in src/registry.rs or file is too small", file=sys.stderr)
        return False
    if not _check_specific_file_content("main", "src/registry.rs", METAEND_FIX, headers, min_length=500):
    ...
    if not _check_specific_file_content("main", "demo/harmony-demo/src/lib/utils.ts", UTILS_CONTENT, headers, min_length=50):
    ...
    if not _check_specific_file_content("main", ".gitignore", GITIGNORE_ADDITION, headers, min_length=100):
    ...
    if not _check_specific_file_content("main", "Cargo.toml", VERSION_110, headers, min_length=200):
    ...
    if not _check_file_content("main", "CHANGELOG.md", CHANGELOG_KEYWORDS, headers):
    ...
    pr_merged, pr_number = _check_pr_merged("Release v1.1.0", "main", headers)
    if not pr_merged:
    ...
    if pr_number and not _check_pr_squash_merged(pr_number, headers):
