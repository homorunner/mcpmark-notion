    ...
    try:
        with sync_playwright() as p:
            ...
            for link in nav_links:
                ...
                if href and ("forms" in href.lower() or href.endswith("/forms/") or href.endswith("/forms")):
...
def test_reverse_navigation() -> Dict[str, Any]:
    ...
    try:
        with sync_playwright() as p:
            ...
            for link in nav_links:
                href = link.get_attribute("href")
                if href and ("navigation" in href.lower() or href.endswith("/navigation")):
            ...
            return {
                "success": True,
                "reverse_navigation_success": navigation_link_found or "navigation" in final_url.lower(),
                "final_url": final_url,
...
def compare_mcp_vs_independent(mcp_results: Dict, navigation_data: Dict, reverse_data: Dict) -> Dict[str, Any]:
    ...
    if expected_pages:
        pages_accuracy = len(mcp_pages.intersection(expected_pages)) / len(expected_pages)
    ...
    nav_accuracy = 1.0 if (mcp_navigation and actual_navigation) or (not mcp_navigation and not actual_navigation) else 0.0
    ...
    reverse_accuracy = 1.0 if (mcp_reverse and actual_reverse) or (not mcp_reverse and not actual_reverse) else 0.0
...
def verify_page_content(page_data: Dict[str, Any]) -> bool:
    ...
    if not page_data.get("success"):
        print(f"❌ Independent verification failed: {page_data.get('error', 'Unknown error')}")
        return False
    ...
    if len(found_expected_links) == len(EXPECTED_NAVIGATION_LINKS):
        print(f"✅ All {len(EXPECTED_NAVIGATION_LINKS)} expected navigation links found")
    else:
        missing_links = [link for link in EXPECTED_NAVIGATION_LINKS if link not in found_expected_links]
        print(f"❌ Missing navigation links: {missing_links}")
        print(f"   Found {len(found_expected_links)}/{len(EXPECTED_NAVIGATION_LINKS)} expected links")
        success = False
...
def verify_task() -> bool:
    ...
    if reverse_data.get("success"):
        ...
        print(f"❌ Reverse navigation test error: {reverse_data.get('error', 'Unknown error')}")
        reverse_success = False
    ...
    if not overall_success:
        ...
        print(f"   Expected: navigation and forms pages")
        print(f"   MCP Navigation Success: {mcp_data['findings']['navigation_successful']}")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("Both navigation functionality and MCP agent accuracy meet requirements")
            sys.exit(0)
        else:
            print("\n❌ Web navigation task verification: FAILED")
            print("Either navigation functionality or MCP agent accuracy below requirements")
            sys.exit(1)
